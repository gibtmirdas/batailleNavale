	...
	public void init() {
		...
		player2.sendMessage(new PacketNewGame(0, round_time));
                for(int i = 0; i < 3;i++){
                    ...
                    player1.sendMessage(new PacketInfoBoat(0 , i + 1,(i + i * 3) , (i + i*3), (i + i * 3+1) , (i + i*3), 3 ));
                    p1BoatLife[i]=3;
                    p2BoatLife[i]=3;
                    gameboard[i+i*3][i+i*3] = i+1;
                    gameboard[i+i*3+1][i+i*3] = i+1;
                    player2.sendMessage(new PacketInfoBoat(0 , i + 4,(i + i * 3) , (i + i*3), (i + i * 3+1) , (i + i*3), 3 ));
	...
	public void packetReceivedCardAction(PacketCardAction p) {
            ...
            if(gameboard[x][y] != 0)
            {
                if(p.getIdSource() == 1)
                {
                    ...
                    player2.sendMessage(new PacketInfoBoat(0, gameboard[x][y],0,0,0,0, p2BoatLife[gameboard[x][y]-3]));
                }else{
                    p1BoatLife[gameboard[x][y]]--;
                    player1.sendMessage(new PacketInfoBoat(0, gameboard[x][y],0,0,0,0, p1BoatLife[gameboard[x][y]]));
                }
                this.checkVictory();
            }            
        }
