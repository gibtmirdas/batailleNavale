...
public class LauncherShopCardList extends JScrollPane implements
		PanelNotifiable {
	...
	public void resizeMyAssNigger(int width, int comp_per_lines) {
		int height = (int) (Math.ceil((double) p.getComponentCount()
				/ (double) (comp_per_lines))
				* (cardHeight + margin) + margin);
	...
	public void receivePacket(Packet p) {
		...
		try {
			...
			addCard(new CardPanel(FactoryCarte.getCarte(tc.getById(pnc
					.getCardId()))));
	...
	private class CardPanel extends JPanel implements MouseListener {
		...
		private static final long serialVersionUID = 1L;
		String[] ids = { "Carte Missile", "Carte Radar", "Carte de ouf",
				"Yassine", "Idiot" };
		int cardId;
		...
		public void makeTargeted(Boolean targeted) {
			if (targeted) {
				this.setBorder(BorderFactory.createBevelBorder(1, Color.BLACK,
						Color.GRAY));
			} else {
				this.setBorder(null);
			}
